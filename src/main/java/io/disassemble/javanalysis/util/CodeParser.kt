...
object CodeParser {
    ...
    fun parse(method: CtMethodNode): List<CtInsn> {
        try {
            ...
            while (itr.hasNext()) {
                ...
                if (InsnUtil.isLdcInsn(opcode)) {
                    ...
                    val `var` = if (InsnUtil.isUnderscoreInsn(opcode)) InsnUtil.underVal(opcode) else itr.byteAt(poolIndex)
                    insn = VarInsn(method, index, opcode, `var`)
                    ...
                    val `var` = itr.byteAt(poolIndex)
                    val increment = itr.signedByteAt(poolIndex + 1)
                    insn = IncrementInsn(method, index, opcode, `var`, increment)
                    ...
                    if (opcode == INVOKEINTERFACE) {
                        ...
                        insn = MethodInsn(method, index, opcode, parent, name, desc)
                    } else {
                    ...
                    if (opcode == MULTIANEWARRAY) {
                        val dims = -1 // handle?
                        insn = MultiANewArrayInsn(method, index, opcode, type, dims)
                    } else {
                        insn = TypeInsn(method, index, opcode, type)
                    }
                } else if (opcode == WIDE) {
            ...
            throw IllegalStateException("Unable to parse instructions for " + method.source().longName)
        }
